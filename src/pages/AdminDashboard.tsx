...
import { Building2, CreditCard, FileText, LogOut, Search, Settings, ShieldCheck, Users } from 'lucide-react';
      ...
      <div className="container mx-auto max-w-7xl px-4 py-8">
        <div className="mb-8 grid gap-6 lg:grid-cols-[1.05fr_0.95fr]">
          <Card className="border-white/80 bg-white/88 shadow-xl shadow-slate-200/50">
            <CardContent className="p-8">
              ...
              <h1 className="mt-2 font-display text-4xl font-bold text-slate-950">Relatórios, acessos e contas em um só lugar.</h1>
              <p className="mt-4 max-w-2xl text-sm leading-7 text-slate-600">
                Este painel mostra quem respondeu, qual foi o perfil predominante e quais contas têm permissão administrativa para acessar os relatórios.
              </p>
            </CardContent>
          </Card>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {[
              ['Relatórios salvos', reportSummary.total.toString()],
        ...
        <Tabs defaultValue="relatorios" className="space-y-6">
          <TabsList className="rounded-2xl bg-white/88 p-1 shadow-sm">
            <TabsTrigger value="relatorios" className="gap-2 rounded-xl"><FileText className="h-4 w-4" />Relatórios</TabsTrigger>
            ...
            <TabsTrigger value="visao" className="gap-2 rounded-xl"><Building2 className="h-4 w-4" />Visão geral</TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="acessos">
            <Card className="border-white/80 bg-white/88 shadow-xl shadow-slate-200/50">
              <CardHeader>
                <CardTitle>Permissões administrativas</CardTitle>
              </CardHeader>
              <CardContent className="space-y-3">
                {accounts.length === 0 ? (
                  <div className="rounded-2xl bg-slate-50 px-4 py-6 text-sm text-slate-500">
                    ...
                    <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                      ...
                      <div className="flex flex-wrap gap-2">
                        <span className="rounded-full bg-white px-3 py-1 text-xs font-medium text-slate-700 shadow-sm">
                          {account.role === 'company' ? 'Empresa' : 'Pessoa física'}
                        </span>
                        <span className="rounded-full bg-white px-3 py-1 text-xs font-medium text-slate-700 shadow-sm">
          ...
          <TabsContent value="visao">
            <div className="grid gap-6 md:grid-cols-2">
              <Card className="border-white/80 bg-white/88 shadow-xl shadow-slate-200/50">
                ...
                <CardContent className="space-y-3">
                  <div className="rounded-2xl bg-slate-50 px-4 py-4 text-sm text-slate-700">
                    Pessoa física: {accounts.filter((account) => account.role === 'user').length}
                  </div>
                  <div className="rounded-2xl bg-slate-50 px-4 py-4 text-sm text-slate-700">
              ...
              <Card className="border-white/80 bg-white/88 shadow-xl shadow-slate-200/50">
                ...
                <CardContent className="space-y-3">
                  <div className="rounded-2xl bg-slate-50 px-4 py-4 text-sm text-slate-700">
                    Relatórios com perfil principal definido: {reports.filter((report) => report.result.primaryProfile).length}
                  </div>
                  <div className="rounded-2xl bg-slate-50 px-4 py-4 text-sm text-slate-700">
                    Acesso admin pronto para você liberar outras contas quando quiser.
                  </div>
                </CardContent>
              </Card>
            </div>
